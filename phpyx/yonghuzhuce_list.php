...
<body>
 <div class="card">
              <div class="card-header"><h4>用户注册列表</h4></div>
              <div class="card-body">
	...
	for($i=$pagecurrent*$pagelarge-$pagelarge;$i<$ddddd;$i++)
{
  ...
  <tr>
    ...
    <td><a class='<?php if(mysql_result($query,$i,"issh")=="是"){echo "btn btn-success m-r-5";}else{echo "btn btn-danger";}?>' href="sh.php?id=<?php echo mysql_result($query,$i,"id") ?>&yuan=<?php echo mysql_result($query,$i,"issh")?>&tablename=yonghuzhuce" onclick="return confirm('您确定要执行此操作？')"><?php echo mysql_result($query,$i,"issh")?></a></td>
	...
	<a class="btn btn-warning" href="yonghuzhuce_updt.php?id=<?php echo mysql_result($query,$i,"id");?>"><i class="mdi mdi-delete-empty"></i> 修改</a> <a class='btn btn-info' href="yonghuzhucedetail.php?id=<?php echo mysql_result($query,$i,"id");?>" target="_blank"><i class='mdi mdi-arrow-expand-all'></i> 详细</a> </td>
  </tr>
  	<?php
	}
}
  ...
  $.confirm({
        ...
        buttons: {
            confirm: {
                text: '确认',
                action: function(){
                    location.href="del.php?id="+nid+"&tablename=yonghuzhuce";
                }
            },
            cancel: {
                text: '关闭',
